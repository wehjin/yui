...
pub fn textfield(id: i32, label: &str, edit: StringEdit, update: SenderLink<stringedit::Action>) -> ArcYard {
    ...
    arc_yard.before(yard::fill(FillColor::Background, FillGrade::Focus))
}
                ...
                if char_index >= 0 && char_index <= char_count as i32 {
                    if let Some(spot) = edit.read_spot(char_index as usize) {
                        if spot.is_cursor && ctx.focus_id() == self.id {
                            ctx.set_fill_grade(FillGrade::Press, body_bounds.z);
                            ctx.set_fill(FillColor::Background, body_bounds.z);
                        }
                        if spot.char != '\n' {
